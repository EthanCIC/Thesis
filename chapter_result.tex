...
\fig[0.5][fig:compress_amplified_compare][!htb]{compress_zoom_in.png}[放大後之電訊號，紅線為未經調製的雷射頻譜，黑線為兩次調製後的訊號，雖能大致上將頻寬從 10 GHz 壓回 60 MHz，但從圖中可發現，中心以外的頻率仍能測到一些訊號。]
...
\begin{figure}[!hbt]
    ...
    \subcaptionbox
        {第一台 EOM 
        \label{fig:subfig_fig1}}
        {\includegraphics[width=0.45\linewidth]{EOM_1.png}}
    ~~~~
    \subcaptionbox
        {第二台 EOM
        \label{fig:subfig_fig2}}
        {\includegraphics[width=0.45\linewidth]{EOM_2.png}}
    \caption{經過放大器，進入 EOM 用以調製的兩組隨機訊號眼圖}
    \label{fig:amplified_signal}
\end{figure}

\fig[0.5][fig:10gbs_compress_sprectrum][!htb]{temp.png}[10 Gb/s 訊號壓縮後頻譜]
...
\fig[0.75][fig:remaining_mod_g2][!htb]{no_etalon_with_cell.png}[單光子通過 $^{87}Rb$ 氣體管之 $G^{2}(\tau)$ 量測]
...
\fig[0.7][fig:spread_absorption_g2][!htb]{no_etalon_spread.png}[展頻後單光子被部分吸收後 $G^{2}(\tau)$ 量測]
...
\fig[0.5][fig:different_gbs_abs][!htb]{temp.png}[展頻後單光子被部分吸收後之 $G^{2}(\tau)$ 量測]
...
\fig[0.5][fig:laser_no_etalon][!htb]{temp.png}[雷射光相位調製對穿透率之影響]
...
\fig[0.5][fig:no_cell_compress_laser][!htb]{temp.png}[原子吸收對雷射光壓縮品質比較圖]
...
\fig[0.5][fig:prbs_signals][!htb]{temp.png}[PRBS 輸出之訊號]
\fig[0.5][fig:prbs_signals_amplified][!htb]{temp.png}[經放大器之隨機訊號]
