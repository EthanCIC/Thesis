...
\fig[0.75][fig:nocell_compress_with_etalon][!htb]{no_cell_compress_with_etalon.png}[在兩台 EOM 同時開啟時測量 $G^{2}(\tau)$，黑線為沒經過 $^{87}Rb$ 原子氣體管時之量測；紅線為透射 $^{87}Rb$ 原子氣體管之訊號，兩者的比值為 54.3\%。][原子吸收對單光子壓縮比較圖]
...
使用修正後的隨機訊號進行展頻、吸收與壓縮的模擬，可讓計算的結果更貼近實驗的測量，以下將整理理論與實驗的結果整理成\cref{tab:spread_abs} 與\cref{tab:compress_abs}
...
\begin{table}[h]
    \centering
    \caption{展頻後的光經過 $^{87}Rb$ 之透射率（無 Etalon 濾波器）}
    \begin{tabular}{| c | c | c | c | c |}
\hline
         & 修正前理論 & 修正後理論 & 雷射光實驗 & 單光子實驗
    \\ \hline
...
從以上的表可以看出，經過修正後的理論更接近實驗的結果，由此可知，想要有效地使用展頻技術，電訊號的品質起了關鍵的作用，需要有更好的訊號產生器與線材才可以將調製的訊號還原成原始的模樣。
