...
\def\degree    {碩士} % 碩士/博士
\def\studentID {105022555} % XD
\def\titleZh   {二能階原子系統中單光子吸收之量子操控}   % Chinese title
\def\titleEn   {Quantum control of single-photon absorption in two-level atoms} % English title
...
\def\profsZh   {褚志崧} 
\def\profsEn   {Chuu{,} Chih-Sung}
\def\degreedate{中~華~民~國~ 一零八 ~年~七~月} % NTHU demo use O not 零
% \def\degreedate{中~華~民~國~ \zhdigits{108} ~年~七~月} % NTHU demo use O not 零
...
\def\sansfont   {}
\def\monofont   {}
\def\CJKmainfont{[BiauKai.ttf]}
\def\CJKsansfont{[BiauKai.ttf]}
\def\CJKmonofont{[BiauKai.ttf]}


%%%%% Style of fonts and line stretch | 字體大小風格及行距 %%%%%
% Base font
\def\baseFontSize{12pt} % Valid: 8pt, 9pt, 10pt, 11pt, 12pt, 14pt, 17pt, 20pt
\def\baseLineStretch{2} % 行距(倍數)
