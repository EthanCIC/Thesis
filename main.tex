...
\begin{document}
    ...
    \mainmatter
        ...
        % \input{chapter_theory}          % 基本原理介紹
        \input{chapter_simulation}      % 理論模擬
        \input{chapter_method}          % 實驗方法與架設
        \input{chapter_result}          % 實驗結果與討論
...
%        \input{chapter_template_demo}
    \backmatter          % book class 預設\backmatter 在\appendix 後面。因此.cls修改過\appendix 定義
        \input{bibliography}
    \appendix
        \input{appendix_Fabry_Perot}
        \input{appendix_Etalon}
